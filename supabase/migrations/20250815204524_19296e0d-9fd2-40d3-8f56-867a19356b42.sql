...
USING (
  ...
  -- Ou permite mudança apenas para status 'cancelado' (edge functions)
  true
)
WITH CHECK (
  -- Admin autenticado pode atualizar tudo
  (auth.uid() IS NOT NULL) OR 
  -- Ou permite apenas mudança para status 'cancelado'
  (OLD.status = 'confirmado' AND NEW.status = 'cancelado' AND OLD.cpf = NEW.cpf AND OLD.data = NEW.data AND OLD.horario = NEW.horario AND OLD.nome = NEW.nome AND OLD.area = NEW.area)
);